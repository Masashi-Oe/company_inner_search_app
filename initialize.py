...
def initialize_retriever():
    ...
    # チャンク分割用のオブジェクトを作成(変数に変更)（追記：課題2）
    ...
    # ベクターストアを検索するRetrieverの作成(変数に変更)（追記：課題2）
...
def file_load(path, docs_all):
    ...
    if file_extension in ct.SUPPORTED_EXTENSIONS:
        ...
        if file_extension == ".csv" and len(docs) > 1:
            ...
            merged_metadata["source"] = path
            merged_doc = Document(page_content=merged_text, metadata=merged_metadata)
